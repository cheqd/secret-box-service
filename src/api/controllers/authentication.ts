...
import {
  PROPOSAL_MESSAGE_TITLE as TITLE,
  REPLY_PROTECTION_INTERVAL,
} from "../constants";
...
export const handleAuthToken = async (
  ...
  switch (signMode) {
    case SignMode.SIGN_MODE_DIRECT:
      ...
      if (!ok) {
        return ErrorHandler.throw({
          msg: AuthTokenErrorEnum.InvalidSignature,
          statusCode: 400,
        });
      }
    ...
    case SignMode.SIGN_MODE_LEGACY_AMINO_JSON:
      ...
      const aminoMsg = CheqdAminoRegistry.toAmino({
    ...
    default:
      return ErrorHandler.throw({
        msg: AuthTokenErrorEnum.NoSignMode,
        statusCode: 400,
      });
  }
};
...
function checkMsg(decoded: DecodedTxRaw): Error | void {
  ...
  const timeElapsed =
    new Date().getTime() - new Date(description.time).getTime();
